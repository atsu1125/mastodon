...
class ActivityPub::Activity::Follow < ActivityPub::Activity
  ...
  def perform
    ...
    if (!target_account.local? && target_account.locked?) || (target_account.local? && target_account.locked? && !target_account.user.setting_auto_accept_followed) || (target_account.local? && target_account.locked? && target_account.user.setting_auto_accept_followed && !target_account.following?(@account)) || @account.silenced? || (target_account.local? && @account.bot?) || target_account.blocking?(@account) || target_account.domain_blocking?(@account.domain) || (!@account.local? && target_account.local? && target_account.user.setting_confirm_follow_from_remote)
