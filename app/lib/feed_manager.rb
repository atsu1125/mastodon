...
class FeedManager
  ...
  def merge_into_home(from_account, into_account)
    ...
    query        = from_account.statuses.where(visibility: [:public, :unlisted, :private, :unleakable]).includes(:preloadable_poll, reblog: :account).limit(FeedManager::MAX_ITEMS / 4)
